...
public partial class InteractionComponent : ComponentBase
{
	...
	private PlayerCharacter _playerOwner;
	...
	public override void _Ready()
	{
		...
		_playerOwner = GetOwner<PlayerCharacter>();

		if (_playerOwner != null)
		{
			_playerOwner.FocusChanged += _UpdateInteractionContext;
		}
	...
	private void _UpdateInteractionContext(Node3D newFocus, Node3D previousFocus)
	{
		...
		if (newFocus is RigidBody3D r)
		{
			var canEquip = EquipmentComponent.CanEquip(r);
			_interactionContext.Grab = true;
			_interactionContext.Equip = canEquip;
		}
		
		EmitSignal(SignalName.InteractionContextUpdated, _interactionContext);
	}

	private void _ClearInteractionContext()
	{
