...
public partial class GridAroundQuerier : Context
{
    ...
    {
        _points = new();
        GD.Print("attempting to get points in a grid around the querier");
        CallDeferred("_CreatePoints", querier);

        await ToSignal(this, SignalName.PointsCreated);

        GD.Print("testing if generating points finishes before this is printed.");
