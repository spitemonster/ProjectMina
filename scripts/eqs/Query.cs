...
public partial class Query : EQSNode
{
	...
	public async Task<Vector3> RunQuery(AgentComponent querier)
	{
		if (QueryContext == null || QueryTests == null)
		{
			...
			if (Debug)
			{
				GD.Print("Can't run query, missing context or tests");
			}
		...
		foreach (var test in QueryTests)
		{
			...
			GD.Print("test result: ", queryPoints);
		}
		
		GD.Print("in theory done with tests");
	...
	public override string[] _GetConfigurationWarnings()
	{
		...
		if (GetChildCount() > 1)
		{
			...
			{
				GD.Print("forsaken one: ", child.Name);
		...
		{
			GD.Print("type: ", GetChild(0).GetType());
